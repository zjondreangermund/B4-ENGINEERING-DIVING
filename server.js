...
app.get('/api/dashboard', async () => {
  ...
  const opsManagers = await pool.query(`SELECT COALESCE(NULLIF(ops_manager,''),'Unassigned') AS ops_manager, COALESCE(SUM(gross_profit),0) AS gross_profit FROM jobs WHERE ops_manager IS NOT NULL AND ops_manager <> '' GROUP BY ops_manager ORDER BY gross_profit DESC`);
  ...
  try {
    ...
    if (expSheet) {
      for (const row of rowsFromSheet(workbook, expSheet)) {
        ...
        // Executive Summary must use only the "Expenses NO JOB #" amount so job-linked costs are not counted twice.
        const noJobAmount = toNumber(row['Expenses NO JOB #']);
        const hasJobNumber = String(row['Job Number'] || '').trim() !== '';
        if (hasJobNumber || noJobAmount <= 0) continue;
