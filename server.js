...
function normalizeVesselName(value) {
  ...
  if (!key) return null;
  if (key === 'saturn' || key === 'dsvsaturn') return 'DSV Saturn';
  if (key === 'ingwegwe') return 'Ingwegwe';
  if (key === 'ingwena') return 'Ingwena';
  if (key === 'dingy' || key === 'dinghy') return 'Dingy';
  if (key === 'flatcat') return 'Flatcat';
  if (key === 'pumba' || key === 'pumbaa') return 'Pumba';
  return text;
}
function isVesselName(value) {
  return Boolean(normalizeVesselName(value) && ['Dingy','Ingwegwe','DSV Saturn','Flatcat','Ingwena','Pumba'].includes(normalizeVesselName(value)));
}
function cleanDivision(value) {
  const text = String(value || '').trim();
  return isVesselName(text) ? null : (text || null);
}
function chooseFinancialSheet(workbook) {
  const preferred = findSheet(workbook, ['Data', 'Jobs', 'Analysis', 'Profit per Job']);
  if (preferred && !['Vessel Info', 'Vessels', 'Expenses', 'Expense'].map(normalizeHeader).includes(normalizeHeader(preferred))) {
    return { sheetName: preferred, sheet: workbook.Sheets[preferred], header: findHeaderRow(workbook.Sheets[preferred]) };
  }
  let best = null;
  for (const sheetName of workbook.SheetNames) {
    if (['Vessel Info', 'VesselInfo', 'Vessels', 'Expenses', 'Expense', 'Salaries', 'Salary'].map(normalizeHeader).includes(normalizeHeader(sheetName))) continue;
    const sheet = workbook.Sheets[sheetName];
    const header = findHeaderRow(sheet);
    const result = { sheetName, sheet, header };
    if (!best || header.score > best.header.score) best = result;
  }
...
async function importWorkbook(buffer, filename) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const division = cleanDivision(pick(row, FIELD_ALIASES.division));
    ...
    if (jrSheet) {
      ...
      for (const row of matrix.slice(2)) {
        ...
        const division = cleanDivision(row[3]);
