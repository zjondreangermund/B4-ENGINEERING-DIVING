...
async function syncFinancialJobFromAcceptedCard(clientOrPool, jobNumber) {
  ...
  const cardsResult = await db.query(`SELECT * FROM job_card_entries WHERE job_number=$1 AND COALESCE(source,'import')='manual' AND review_status='accepted' ORDER BY job_date ASC NULLS LAST, id ASC`, [cleanJobNumber]);
...
async function recomputeWorkshopAndFinancialJobsForDate(jobDate) {
  ...
  // Excel formula: Workshop Cost = row hours / SUMIF(all conversion rows on same date, hours) * (annual workshop cost / 365).
  // Keep imported Excel rows unchanged, but include their hours in the denominator so newly-created app cards
  // receive the same fair daily workshop share instead of the full daily workshop pool.
  ...
  const manualRows = allRowsForDate.rows.filter(row => String(row.source || 'import') === 'manual' && String(row.review_status || 'pending') === 'accepted');
  const totalHours = allRowsForDate.rows.reduce((sum, row) => sum + Math.max(0, Number(row.hours || 0)), 0);
