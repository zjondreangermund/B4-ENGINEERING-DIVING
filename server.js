...
async function importWorkbook(buffer, filename) {
  ...
  let importedRows = 0, skippedRows = 0, registerRows = 0, cardRows = 0, salaryRows = 0, vesselRows = 0;
  try {
    ...
    return { status: 'imported', filename, sheet: best.sheetName, headerRow: best.header.rowIndex + 1, headerScore: best.header.score, rowsFound: rows.length, importedRows, skippedRows, registerRows, cardRows, salaryRows, vesselRows, sheets: workbook.SheetNames };
  } catch (error) {
