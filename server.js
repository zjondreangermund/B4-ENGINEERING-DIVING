...
function calculateJobCardConversionCost(card) {
  ...
  const equipmentHourlyCost = equipment.reduce((sum, name) => sum + (rateForName(JOB_CARD_CONVERSION_RATES.equipmentHourly, name) * hours), 0);
  const equipmentFixedCost = equipment.reduce((sum, name) => sum + rateForName(JOB_CARD_CONVERSION_RATES.equipmentFixed, name), 0);
  const equipmentCost = equipmentHourlyCost + equipmentFixedCost;
  return {
    labourCost,
    equipmentCost,
    workshopCost: 0,
