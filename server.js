...
app.get('/api/job-profit', async (request) => {
  ...
  const result = await pool.query(`SELECT
      ...
      COALESCE(c.card_cost,0) AS manual_card_cost,
      COALESCE(c.card_hours,0) AS manual_card_hours,
      COALESCE(j.total_cost,0) + COALESCE(c.card_cost,0) AS total_cost_with_cards,
      COALESCE(j.revenue,0) - (COALESCE(j.total_cost,0) + COALESCE(c.card_cost,0)) AS profit_with_cards
    FROM jobs j
    LEFT JOIN (
      SELECT job_number, SUM(COALESCE(total_cost,0)) AS card_cost, SUM(COALESCE(hours,0)) AS card_hours
