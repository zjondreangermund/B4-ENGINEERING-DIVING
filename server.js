...
app.get('/api/job-cards/pending', async (request, reply) => {
  try {
    ...
    const result = await pool.query(`SELECT * FROM job_card_entries WHERE COALESCE(source,'import')='manual' AND COALESCE(review_status,'pending')='pending' ORDER BY imported_at DESC, id DESC LIMIT 1000`);
    return result.rows;
  } catch (error) {
    app.log.error(error);
    return reply.code(500).send({ error: error.message });
...
app.post('/api/job-cards/manual', async (request, reply) => {
  try {
    ...
    const result = await pool.query(`INSERT INTO job_card_entries
      (job_date, month, year, fy, job_number, ops_manager, description, start_time, end_time, hours, labour_cost, equipment_cost, workshop_cost, material_cost, other_cost, total_cost, division, client_name, location, vessel_name, contact_person, activity_type, staff_names, equipment_used, completed_by_user, completed_by_name, import_key, source, review_status)
