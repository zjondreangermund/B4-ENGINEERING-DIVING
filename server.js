...
async function recomputeWorkshopAndFinancialJobsForDate(jobDate) {
  const iso = toDate(jobDate);
  if (!iso) return;
  const result = await pool.query(`SELECT * FROM job_card_entries WHERE job_date=$1 AND COALESCE(source,'import')='manual' AND review_status='accepted'`, [iso]);
  const totalHours = result.rows.reduce((sum, row) => sum + Math.max(0, Number(row.hours || 0)), 0);
