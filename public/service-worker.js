const CACHE_NAME='b4-ops-cache-v20-force-valid-ops-filters';
...
<script>
(function(){
  if(window.__B4_FILTER_PIPELINE_V20__) return;
  window.__B4_FILTER_PIPELINE_V20__ = true;
  ...
  function arr(name){const value=g(name,[]); return Array.isArray(value) ? value : []}
  ...
  function setSelectOptions(id, first, values){
  ...
  function enforceValidOpsSelect(id){
    const el=document.getElementById(id); if(!el) return;
    const before=el.value;
    setSelectOptions(id,'All OPS managers',VALID_OPS);
    if(before && !VALID_OPS.includes(before)) el.value='';
  }
  function enforceAllOpsFilters(){
    enforceValidOpsSelect('jobOps');
    enforceValidOpsSelect('cardsOpsFilter');
  }

  window.setJobCardFilterOptions = function(){
    enforceValidOpsSelect('cardsOpsFilter');
    setSelectOptions('cardsVesselFilter','All vessels',arr('cards').map(c=>c.vessel_name));
    setSelectOptions('cardsActivityFilter','All activities',arr('cards').map(c=>c.activity_type));
  };
  window.setFinancialJobFilterOptionsV20 = function(){
    setSelectOptions('jobDivision','All divisions',arr('jobs').map(j=>j.division));
    enforceValidOpsSelect('jobOps');
  };
  ...
  window.renderCards = function(){
    ...
    let filteredRows=arr('cards').filter(r=>{
      ...
      return (!q || hay.includes(q)) && (!src || (src==='manual' ? isManual : !isManual)) && (!ops || nf(r.ops_manager)===ops) && (!vessel || nf(r.vessel_name)===vessel) && (!activity || nf(r.activity_type)===activity);
    });
  ...
  window.renderJobs = function(){
    ...
    let filteredRows=arr('jobs').filter(j=>{
  ...
  window.renderRegister = function(){
    ...
    let base=arr('registerSearchResults');
    if(!base.length) base=arr('registers');
  ...
  function bind(ids,page,renderFn){
    ids.forEach(id=>{
      const el=document.getElementById(id); if(!el || el.dataset.v20Bound==='yes') return;
  ...
  function apply(){
    try{ window.setFinancialJobFilterOptionsV20(); window.setJobCardFilterOptions(); enforceAllOpsFilters(); }catch(e){console.warn('B4 v20 option setup failed',e)}
    ...
    try{window.renderCards();window.renderJobs();window.renderRegister();}catch(e){console.warn('B4 v20 render failed',e)}
  }

  const oldLoad=g('loadAll',null);
  if(typeof oldLoad==='function'){
    window.loadAll=async function(){const out=await oldLoad.apply(this,arguments); setTimeout(apply,0); setTimeout(apply,500); return out;};
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(apply,700)); else setTimeout(apply,700);
  setInterval(()=>{try{enforceAllOpsFilters();}catch(e){}},750);
  setTimeout(()=>{try{new MutationObserver(()=>enforceAllOpsFilters()).observe(document.body,{childList:true,subtree:true});}catch(e){}},1000);
})();
</script>`;
...
self.addEventListener('fetch', event => {
  ...
  if (req.mode === 'navigate' || url.pathname === '/' || url.pathname.endsWith('/index.html')) {
    event.respondWith((async () => {
      ...
      if (!html.includes('__B4_FILTER_PIPELINE_V20__')) {
